...
/*   Updated: 2016/03/09 22:37:07 by tdefresn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		set_args(t_list **list, t_list **to_list, t_file_datas *file)
{
	if (*list)
	{
...
void			fetch_args(int argc, char **argv, t_ls_datas *ls_datas)
{
	...
	while (--argc > i)
	{
		...
		if (ret < 0)
		{
			...
			set_args(&dir_list, &ls_datas->directories, &file);
		else if (S_ISREG(st_stat.st_mode))
			set_args(&files_list, &ls_datas->files, &file);
